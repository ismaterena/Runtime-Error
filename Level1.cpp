...
// =========================================================
// NAMESPACE: Isolates Level 1 so it doesn't crash other levels
// =========================================================
namespace L1 {
...
#define MAX_PROJECTILES 8
	...
	// RENAMED TO AVOID COLLISION WITH GLOBALS.H
	enum BattleState { PRE_BATTLE, PLAYING, POST_BATTLE_WIN, POST_BATTLE_LOSE };
	...
	struct Projectile {
		...
		void update() {
			if (!active) return;
			x += speed * direction;
			if (x < cameraX - 200 || x > cameraX + SCREEN_WIDTH + 200) active = false;
		}
	...
	struct Enemy {
		...
		void draw() {
			int drawX = x - cameraX;
			if (throwTimer < 25 && battleState == PLAYING) iShowImage(drawX, y, width, height, facingRight ? imgThrowR : imgThrowL);
			else if (isJumping) {
			...
			else if (isMoving && battleState == PLAYING) iShowImage(drawX, y, width, height, facingRight ? imgRunR : imgRunL);
	...
	struct Player {
		...
		void update() {
			...
			cameraX = x - (SCREEN_WIDTH / 2) + (width / 2);
		...
		void draw() {
			...
			if (isAttacking && battleState == PLAYING) {
				iSetColor(0, 255, 255);
				if (facingRight) {
	...
	void resetLevel1() {
		...
		cameraX = 0; battleState = PRE_BATTLE; dialogueStep = 0;
	}
	...
	void drawPrologueSystem() {
		...
		if (scene == 1) {
			...
			drawPNG(400, 150, 200, 350, displayImg);
	...
	void drawLevel1System() {
		...
		if (battleState == PLAYING) drawHUD();
		else if (battleState == PRE_BATTLE) {
		...
		else if (battleState == POST_BATTLE_WIN) {
		...
		else if (battleState == POST_BATTLE_LOSE) {
	...
	void update() {
		if (masterState == GAME_MENU) {
			...
			if (slideOffset == 0) {
				...
				if (GetAsyncKeyState(VK_RETURN)) {
					if (!enterPressed) {
						if (currentMenu == MAIN_MENU) {
							if (selectedOption == 0) {
								masterState = GAME_PROLOGUE; scene = 1; scene1Step = 0;
							}
							// NEW FEATURE: "CONTINUE" OPTION!
							else if (selectedOption == 1) {
								if (::unlockedLevel == 2) ::currentState = LEVEL_2;
								else if (::unlockedLevel == 3) ::currentState = LEVEL_3;
							}
			...
			if (battleState == PLAYING) {
				...
				if (playerHP <= 0) { dialogueStep = 0; battleState = POST_BATTLE_LOSE; }

				frameCounter++;
				if (frameCounter >= 60) {
					gameTime--; frameCounter = 0;
					if (gameTime <= 0) { dialogueStep = 0; battleState = POST_BATTLE_WIN; }
				}
			}
		}
	}
	...
	void advanceNarrative() {
		if (masterState == GAME_PROLOGUE) {
			if (scene == 1) { scene1Step++; if (scene1Step > 7) { scene = 2; scene2Step = 0; } }
				...
				scene5Step++;
				if (scene5Step > 3) {
					masterState = GAME_LEVEL1; resetLevel1();
				}
			}
		}
		else if (masterState == GAME_LEVEL1) {
			if (battleState == PRE_BATTLE) {
				dialogueStep++; if (dialogueStep > 2) battleState = PLAYING;
			}
			else if (battleState == POST_BATTLE_WIN || battleState == POST_BATTLE_LOSE) {
				dialogueStep++;
				if (dialogueStep > 2) {
					if (battleState == POST_BATTLE_WIN) {
						// THE MAGIC BULLET: Win Level 1 -> Go to Level 2!
						::unlockedLevel = 2;
						saveProgress(2);
						::currentState = LEVEL_2; // Switches the master director!
					}
					else {
						// Lost? Go back to Main Menu
						masterState = GAME_MENU; currentMenu = MAIN_MENU;
						slideOffset = 1000; isTransitioning = false;
					}
				}
			}
	...
	void mouse(int button, int state, int mx, int my) {
		if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
				...
				if (!isTransitioning && slideOffset == 0 && currentMenu == MAIN_MENU) {
					for (int i = 0; i < TOTAL_OPTIONS; i++) {
						...
						if (mx > 50 && mx < 340 && my > baseY && my < baseY + 45) {
							selectedOption = i;
							if (i == 0) { masterState = GAME_PROLOGUE; scene = 1; scene1Step = 0; }
							else if (i == 1) {
								if (::unlockedLevel == 2) ::currentState = LEVEL_2;
								else if (::unlockedLevel == 3) ::currentState = LEVEL_3;
							}
			...
			else if (masterState == GAME_LEVEL1 && battleState != PLAYING) advanceNarrative();
		}
	}
...
// =========================================================
// INTERFACE: Connects Level 1 to your Main Director File
// =========================================================
void initLevel1() { L1::init(); }
void drawLevel1() { L1::draw(); }
void updateLevel1() { L1::update(); }
void keyboardLevel1(unsigned char key) { L1::keyboard(key); }
void mouseLevel1(int button, int state, int mx, int my) { L1::mouse(button, state, mx, my); }
void passiveMouseLevel1(int mx, int my) { L1::passiveMouse(mx, my); }